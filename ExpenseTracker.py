...
myb = mysql.connector.connect(host="localhost", user="root", passwd="xuprT44@stuti", database="ExpenseTracker")
...
def signup():
    ...
    if (nameval()):
        if (already()):
            t = user_input.get().strip()
            str1 = "Create table " + t + "(DATE_OF_EXPENSE date,TITLE varchar(20),MONEY int)"
...
def click_weekly():
    ...
    mycursor.execute(
        "Select TITLE, sum(Money) TOTAL_EXPENSE from " + en + " where DATE_OF_EXPENSE between DATE_SUB( curdate(), INTERVAL 7 DAY ) and curdate() group by Title");
    ...
    x, p, texts = plt.pie(slices / slices.sum(), colors=colors, radius=1.2, autopct="%1.1f%%", normalize=False)
...
def click_monthly():
    ...
    colors = ['Blue', 'Yellow', 'Green', 'Red', 'Orange', 'lightblue', 'pink', 'Purple']
    slices=np.array(slices)
    x, p, texts = plt.pie(slices / slices.sum(), colors=colors, radius=1.2, autopct="%1.1f%%", normalize=False)  # fixing radius and all
